...
class LocalBlend:
    def __init__(self, prompts: List[str], words, threshold: float = .3, device: str = 'cpu', tokenizer=None, max_number_of_words=77):
        ...
        alpha_layers = torch.zeros(
            len(prompts),  1, 1, 1, 1, self.max_number_of_words)
    ...
    def __call__(self, x_t, attention_store, step):
        k = 1
        maps = attention_store["down_cross"][:2] + \
            attention_store["up_cross"][3:6]
        maps = [item.reshape(self.alpha_layers.shape[0], -1, 1,
                             16, 16, self.max_number_of_words) for item in maps]
...
class AttentionControlEdit(AttentionStore, abc.ABC):
    def __init__(self, prompts, num_steps: int,
        ...
        self.cross_replace_alpha = ptp_utils.get_time_words_attention_alpha(
            prompts, num_steps, cross_replace_steps, self.tokenizer).to(self.device)
...
class AttentionRefine(AttentionControlEdit):
    def __init__(self, prompts, num_steps: int, cross_replace_steps: float, self_replace_steps: float,
                 local_blend: Optional[LocalBlend] = None, tokenizer=None, device: str = 'cpu'):
