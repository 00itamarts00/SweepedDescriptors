...
class InterpolateKeyframes(nn.Module):
    ...
    def setup(self):
        if self.method == 'ifrnet' and self.model is None:
            ...
            self.model.load_state_dict(torch.load(f'./{IFRNET_PRETRAINED_WEIGHTS}', map_location=torch.device(self.device)))
    ...
    def ifrnet_interp(self, a, b, num_frames):
        ...
        embts = torch.cat([torch.tensor(i).view(1, 1, 1, 1) for i in  torch.linspace(0, 1, num_frames)[1:-1]], 0)
    ...
    def forward_ifrnet(self, keyframes):
        ...
        for a, b in tqdm(zip(keyframes[:-1], keyframes[1:]), total=len(keyframes)):
            vid.append(a)
...
if __name__ == '__main__':
    ...
    keyframes = np.random.randint(low=0, high=255, size=(5, 224, 224, 3)).astype(np.uint8)
    keyframes = [i for i in keyframes]

    vid = interpolator.forward_ifrnet(keyframes)
    
    fourcc = cv2.VideoWriter_fourcc(*'MP42')
    video_writer = cv2.VideoWriter('test.mp4', fourcc, float(interpolator.fps), (224, 224))
